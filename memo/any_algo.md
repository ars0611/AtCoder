...
## 三分探索   
- 凸関数最小値典型   
- f(n)が最小値をとるnが`l <= n <= r`に存在することが保証されるl, rを近づけていく   
1. l, rの初期値を設定   
1. `m_1 = (2 * l + r) // 3`, `m_2 = (l + 2 * r) // 3`とする   
1. f(m_1) < f(m_2) => r = m_2、偽ならば l = m_1 を閉区間[l, r]が十分小さくなるまで繰り返す   
1. 十分小さな区間内で最小値を求める
- いつかライブラリを生やします   
## 連続部分列
...
- 愚直だとO(n*2**k)をO(n*k)に落とせるのがナップサックDP   
...
- 前から愚直に文字列を見ていく際、今**何重のかっこの中身を見ているのか**管理するのも典型テク   
## 繰り返し二乗法   
